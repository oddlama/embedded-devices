//! # SCD41
//!
//! The SCD41 is a photoacoustic NDIR CO2 sensor from Sensirion's SCD4x family which features
//! improved accuracy, large measurement range of 400 - 5000 ppm, single shot operation feature and
...
//! let mut scd41 = SCD41Sync::new_i2c(delay, i2c, Address::Default);
//! scd41.init()?;
//! scd41.start_measuring()?;
//!
//! // Read measurements
//! let measurement = scd41.next_measurement()?;
...
//!
//! // Create and initialize the device
//! let mut scd41 = SCD41Async::new_i2c(delay, i2c, Address::Default);
//! scd41.init().await?;
//! scd41.start_measuring().await?;
//!
//! // Read measurements
//! let measurement = scd41.next_measurement().await?;
//! let humidity = measurement.relative_humidity.get::<percent>();
//! let temperature = measurement.temperature.get::<degree_celsius>();
//! let co2 = measurement.co2_concentration.get::<part_per_million>();
//! println!("Current measurement: {:?}%RH, {:?}°C, {:?}ppm CO₂", humidity, temperature, co2);
...
pub struct SCD41<D: hal::delay::DelayNs, I: embedded_registers::RegisterInterface> {
    /// The delay provider
    delay: D,
...
impl<D, I> SCD41<D, embedded_registers::i2c::I2cDevice<I, hal::i2c::SevenBitAddress>>
where
...
impl<D: hal::delay::DelayNs, I: embedded_registers::RegisterInterface> SCD41<D, I> {
...
impl<D: hal::delay::DelayNs, I: embedded_registers::RegisterInterface> crate::device::ResettableDevice for SCD41<D, I> {
...
impl<D: hal::delay::DelayNs, I: embedded_registers::RegisterInterface> crate::sensor::ContinuousSensor for SCD41<D, I> {
    type Error = TransportError<I::BusError>;
...
impl<D: hal::delay::DelayNs, I: embedded_registers::RegisterInterface> crate::sensor::OneshotSensor for SCD41<D, I> {
    type Error = TransportError<I::BusError>;
    type Measurement = Measurement;
